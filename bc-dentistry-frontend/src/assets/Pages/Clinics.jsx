...
export default function Clinics() {
  ...
  const toggle=async(clinic)=>{
    setError('');
    try{await request(`/clinics/${clinic.clinicID}`,{method:'PATCH',body:JSON.stringify({...clinic,isActive:!clinic.isActive})});await load();}
  ...
  const adminInput=(label,key,type='text')=><label className="block">{label}<input type={type} required value={form.admin[key]} onChange={(e)=>setInitialAdmin(key,e.target.value)} className="mt-1 w-full rounded border p-2" /></label>;
  const managedInput=(label,key,type='text')=><label className="block">{label}<input type={type} required value={admin?.[key]||''} onChange={(e)=>setAdmin({...admin,[key]:e.target.value})} className="mt-1 w-full rounded border p-2" /></label>;
  const transferInput=(label,key,type='text')=><label className="block">{label}<input type={type} required value={transfer[key]} onChange={(e)=>setTransfer({...transfer,[key]:e.target.value})} className="mt-1 w-full rounded border p-2" /></label>;
  ...
  return <section className="p-4 md:p-8">
    ...
    {selected&&admin&&<section className="mt-8 rounded-xl bg-white p-6 shadow" aria-labelledby="manage-admin-title">
      ...
      <form onSubmit={resetAdminPassword} className="mt-8 border-t pt-6"><h3 className="text-lg font-semibold">Issue temporary password</h3><p className="mt-1 text-sm text-gray-600">This revokes all active sessions and requires a password change at next login.</p><label className="mt-3 block max-w-xl">Temporary password<input type="password" required value={resetPassword} onChange={(e)=>setResetPassword(e.target.value)} className="mt-1 w-full rounded border p-2" /></label><button disabled={saving} className="mt-3 rounded border border-blue-700 px-4 py-2 text-blue-800">Reset password and revoke sessions</button></form>
