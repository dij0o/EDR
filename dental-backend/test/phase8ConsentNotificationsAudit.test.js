...
test('Phase 8 access requests capture who, what, when, why, and notify admins', () => {
...
test('Phase 8 admin and patient decisions create notifications and support revocation', () => {
  ...
  assert.match(api, /app\.post\(\['\/revokeConsent', '\/patient\/revokeConsent'\]/);
  assert.match(api, /submitTransaction\(\s*'RevokeConsent'/);
  assert.match(mobileApproved, /revokeConsent/);
  assert.match(mobileApproved, /\/revokeConsent/);
});

test('Phase 8 notification and audit APIs are exposed to authenticated owners', () => {
...
test('Phase 8 patient-facing request details are no longer static placeholders', () => {
