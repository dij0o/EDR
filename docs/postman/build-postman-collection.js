...
const chain = '{{blockchainApiBaseUrl}}';
...
const web = [
  ...
  },
  {
    ...
    item: [
      ...
      request('Create Doctor', 'POST', db, '/doctors',
        'Creates a doctor in MySQL and Fabric for the authenticated clinic admin.',
  ...
  },
  {
    ...
    item: [
      ...
      request('Create Patient', 'POST', db, '/patients',
        'Clinic-admin creation of a patient in MySQL and Fabric.',
      ...
      request('Assign Patient to Doctor', 'POST', db, '/patients/{{patientId}}/assign',
        'Assigns a clinic patient to a doctor in the authenticated admin’s clinic.',
  ...
  },
  {
    ...
    item: [
      request('List Requests for Clinic Admin', 'GET', chain, '/getRequestsForAdmin/{{clinicId}}', 'Lists cross-clinic data-access requests requiring or involving the authenticated clinic admin.'),
      request('Approve Request as Admin', 'POST', chain, '/approveRequest',
      ...
      request('Reject Request as Admin', 'POST', chain, '/admin/rejectRequest',
      ...
      request('Request Patient Data Access', 'POST', chain, '/requestAccess',
      ...
      request('Get Clinical Access Audit', 'GET', chain, '/audit/clinical-access/{{patientId}}',
  ...
  },
  {
    ...
    item: [
      request('List Patient Radiographic Files', 'GET', chain, '/patients/{{patientId}}/radiographic-files', 'Lists Fabric-anchored metadata for a patient’s radiographic/DICOM files.'),
      request('Upload Radiographic File', 'POST', chain, '/radiographic-files',
      ...
      request('Verify Radiographic File Integrity', 'GET', chain, '/radiographic-files/{{fileId}}/verify-integrity', 'Recomputes the stored file hash and compares it with Fabric metadata.'),
      request('Download/View Radiographic File', 'GET', chain, '/radiographic-files/{{fileId}}/content?purpose=radiographic%20image%20view', 'Streams a file only after integrity verification and logs the access purpose.'),
    ],
  },
  {
    name: 'Notifications & Push Devices',
    item: [
      request('List Notifications', 'GET', chain, '/notifications?status=ALL', 'Lists notifications for the authenticated actor. status may be ALL, READ, or UNREAD.'),
      request('Mark Notification Read', 'POST', chain, '/notifications/{{notificationId}}/read', 'Marks one notification as read.'),
      request('Get Push Configuration', 'GET', chain, '/push/config', 'Returns server push readiness and token staleness configuration.'),
      request('List Push Subscriptions', 'GET', chain, '/push/subscriptions', 'Lists notification device registrations owned by the authenticated actor.'),
      request('Register Web Push Token', 'POST', chain, '/push/subscriptions',
      ...
      request('Unregister Current Push Token', 'DELETE', chain, '/push/subscriptions',
        'Unregisters a push token, normally during logout.',
        { body: { token: '{{pushToken}}' } }),
      request('Remove Push Subscription by ID', 'DELETE', chain, '/push/subscriptions/{{subscriptionId}}', 'Removes a selected notification device belonging to the authenticated actor.'),
    ],
  },
];
...
const mobile = [
  {
    ...
    item: [
      request('Patient Login', 'POST', db, '/login',
        '**Purpose:** Authenticate a patient and obtain the JWT required by every protected mobile API.\n\n**Current behavior:** Returns `{ token, user }`. The app rejects non-patient users after login. The test script stores the JWT and patient blockchain ID as collection variables.\n\n**Lifecycle note:** The current app keeps this token only in React state; secure persistence/session restoration is not yet implemented.',
  ...
  },
  {
    ...
    item: [
      request('List All Patient Data Requests', 'GET', chain, '/getAllRequestsForPatient/{{patientId}}',
      ...
      request('Grant Consent', 'POST', chain, '/grantConsent',
      ...
      request('Reject Data Request', 'POST', chain, '/patient/rejectRequest',
      ...
      request('Revoke Previously Granted Consent', 'POST', chain, '/patient/revokeConsent',
...
const collection = {
  ...
  item: [
    ...
    { name: 'Mobile', description: 'APIs invoked by the Expo patient mobile app, plus clearly labeled required session-contract gaps.', item: mobile },
  ],
  variable: [
    { key: 'databaseApiBaseUrl', value: 'http://localhost:3001', type: 'string' },
    { key: 'blockchainApiBaseUrl', value: 'http://localhost:3000', type: 'string' },
    { key: 'webOrigin', value: 'http://localhost:5173', type: 'string' },
