...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-tools:${IMAGETAG} peer version | sed -ne 's/^ Version: //p')
...
if [[ "$DOCKER_SOCK" == "/var/run/docker.sock" && ( "$OSTYPE" == msys* || "$OSTYPE" == cygwin* ) ]]; then
