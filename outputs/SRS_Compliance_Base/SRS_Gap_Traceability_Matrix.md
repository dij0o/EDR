...
| FR-29 | Admin create appointments linking patient, doctor, datetime, specialty. | Deployed and verified | Admin-only `POST /appointments`, migration, scoped doctor options, and persisted web dialog are live; AWS smoke created an assigned-patient appointment. | Complete; authoritative legacy doctor clinic backfill remains separate from the assignment fallback. |
| FR-30 | Patients view upcoming/past appointments in mobile by specialty/date. | Deployed and verified at API/source | JWT patient-bound `GET /appointments?period=upcoming|past` and mobile specialty/date grouping replace static data; AWS smoke verified both partitions. | Capture packaged mobile-device evidence in Phase 10/12. |
| FR-31 | Admin update/cancel appointments. | Deployed and verified | Clinic-scoped `PUT /appointments/:id` and `PATCH /appointments/:id/cancel` persist updates and cancellation evidence; web tickets invoke both. | Complete. |
