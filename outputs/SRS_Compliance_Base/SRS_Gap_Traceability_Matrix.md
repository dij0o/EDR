...
| FR-13 | Doctor retrieve assigned patients via `GetPatientsAssignedToDoctor`. | Implemented at source | UI calls parameter-free `GET /doctor/me/assigned-patients`; API derives the ID from JWT and chaincode verifies certificate actorID. Source regression tests cover absence of route/body identity input. | Deploy and verify self success plus spoof denial on AWS. |
...
| Containerized deployment | Gap To Close | SRS expects application services to run as containerized services. Current/deployed history included at least one non-Fabric service running host-side under PM2; Phase 11 must containerize every non-Fabric service and document the Fabric/Hyperledger exception boundary. |
