...
- Phase 3 deployment gate: no DB migration is required. Deploy/restart the Blockchain API and package/approve/commit the updated chaincode, then smoke-test every canonical route and legacy alias. The current session reached the AWS SSH endpoint but its available key was rejected, so Phase 3 is not yet recorded as deployed.
